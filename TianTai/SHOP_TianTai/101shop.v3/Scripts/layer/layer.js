...
; !function (window, undefined) {
    ...
    Class.pt.move = function () {
        ...
        $(config.move).bind('mousedown', function (M) {
            M.preventDefault();
            if ($(this).attr('move') === 'ok') {
                conf.ismove = true;
                conf.layero = $(this).parents('.' + doms[0]);
    ...
    Class.pt.callback = function () {
        ...
        //按钮
        layero.find('.' + doms[6]).children('a').bind('click', function () {
            var index = $(this).index();
            if (index === 0) {
                config.yes ? config.yes(that.index, layero) : layer.close(that.index);
            } else if (index === 1) {
        ...
        //右上角关闭回调
        layero.find('.' + doms[7]).bind('click', cancel);

        //点遮罩关闭
        if (config.shadeClose) {
            $('#layui-layer-shade' + that.index).bind('click', function () {
                layer.close(that.index);
            });
        }

        //最小化
        layero.find('.layui-layer-min').bind('click', function () {
            layer.min(that.index, config);
            config.min && config.min(layero);
        });

        //全屏/还原
        layero.find('.layui-layer-max').bind('click', function () {
    ...
    Class.pt.openLayer = function () {
        ...
        layer.setTop = function (layero) {
            ...
            layero.bind('mousedown', setZindex);
            return layer.zIndex;
        };
