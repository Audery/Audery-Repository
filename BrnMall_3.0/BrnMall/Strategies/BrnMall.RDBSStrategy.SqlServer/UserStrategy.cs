...
namespace BrnMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public DataTable AdminGetUserList(int pageSize, int pageNumber, string condition)
        {
            ...
            else
            {
                if (noCondition)
                    ...
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[storeid],[{1}users].[mallagid],[{1}users].[nickname],[{1}users].[paycredits],[{1}users].[rankcredits],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}malladmingroups].[title] AS [atitle] FROM [{1}users],[{1}userdetails],[{1}userranks],[{1}malladmingroups]  WHERE [{1}userdetails].[uid] = [{1}users].[uid] AND  [{1}userranks].[userrid]=[{1}users].[userrid] AND  [{1}malladmingroups].[mallagid]=[{1}users].[mallagid] AND [{1}users].[uid] < (SELECT min([uid])  FROM (SELECT TOP {2} [uid] FROM [{1}users] WHERE {3} ORDER BY [uid] DESC) AS temp ) AND {4} ORDER BY [{1}users].[uid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                condition);
        ...
        public void SetStoreAdminer(int uid, int storeId)
        {
            DbParameter[] parms = {
                                    GenerateInParam("@uid", SqlDbType.Int, 4, uid),
                                    GenerateInParam("@storeid", SqlDbType.Int, 4, storeId)
                                   };
            string commandText = string.Format("UPDATE [{0}users] SET [storeid]=0 WHERE [storeid]=@storeid;UPDATE [{0}users] SET [storeid]=@storeid WHERE [uid]=@uid;",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable GetMallAdminGroupList()
        {
            string commandText = string.Format("SELECT {0} FROM [{1}malladmingroups]",
                                                RDBSFields.MALL_ADMINGROUPS,
                                                RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        /// </summary>
        /// <returns></returns>
        public IDataReader GetMallAdminActionList()
        {
            string commandText = string.Format("SELECT {0} FROM [{1}malladminactions] ORDER BY [displayorder] DESC",
                                                RDBSFields.MALL_ADMINACTIONS,
                                                RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        /// <param name="productName">商品名称</param>
        /// <returns></returns>
        public int GetFavoriteProductCount(int uid, string storeName, string productName)
        {
            DbParameter[] parms = {
                                        GenerateInParam("@uid", SqlDbType.Int, 4, uid),
                                        GenerateInParam("@storename", SqlDbType.NChar, 60, storeName),
        ...
        /// <param name="storeId">店铺id</param>
        /// <returns></returns>
        public bool DeleteFavoriteStoreByUidAndStoreId(int uid, int storeId)
        {
            DbParameter[] parms = {
                                     GenerateInParam("@uid", SqlDbType.Int, 4, uid),
                                     GenerateInParam("@storeid", SqlDbType.Int, 4, storeId)
            ...
            return RDBSHelper.ExecuteNonQuery(CommandType.StoredProcedure,
                                              string.Format("{0}deletefavoritestorebyuidandstoreid", RDBSHelper.RDBSTablePre),
                                              parms) > 0;
        }
        ...
        public bool IsExistFavoriteStore(int uid, int storeId)
        {
            ...
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.StoredProcedure,
                                                                   string.Format("{0}isexistfavoritestore", RDBSHelper.RDBSTablePre),
                                                                   parms)) > 0;
        }
        ...
        public DataTable GetFavoriteStoreList(int pageSize, int pageNumber, int uid)
        {
            ...
            return RDBSHelper.ExecuteDataset(CommandType.StoredProcedure,
                                             string.Format("{0}getfavoritestorelist", RDBSHelper.RDBSTablePre),
                                             parms).Tables[0];
        }
        ...
        /// <param name="uid">用户id</param>
        /// <returns></returns>
        public int GetFavoriteStoreCount(int uid)
        {
            DbParameter[] parms = {
                                        GenerateInParam("@uid", SqlDbType.Int, 4, uid)
                                    };
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.StoredProcedure,
                                                                   string.Format("{0}getfavoritestorecount", RDBSHelper.RDBSTablePre),
                                                                   parms));
        }
